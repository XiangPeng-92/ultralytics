...
class MarkdownLinkFixer:
    ...
    def replace_front_matter(content, lang_dir):
        ...
        translations = {
            "zh": [
                ...
                "描述",
                "关键词",
            ],  # Mandarin Chinese (Simplified) warning, sometimes translates as 关键字
        ...
        for term, eng_key in zip(translations.get(lang_dir.stem, []), english):
            content = (
                re.sub(
                    rf"{term} *[：:].*", f"{eng_key}: true", content, flags=re.IGNORECASE
                )
                if eng_key == "comments"
                else re.sub(
                    rf"{term} *[：:] *", f"{eng_key}: ", content, flags=re.IGNORECASE
                )
            )
        return content
    ...
    def link_replacer(self, match, parent_dir, lang_dir, use_abs_link=False):
        ...
        if not linked_path.exists():
            en_linked_path = Path(
                str(linked_path).replace(str(lang_dir), str(lang_dir.parent / "en"))
            )
            if en_linked_path.exists():
                if use_abs_link:
                    # Use absolute links WARNING: BUGS, DO NOT USE
                    docs_root_relative_path = en_linked_path.relative_to(
                        lang_dir.parent
                    )
                    ...
                    updated_path = Path("../" * steps_up) / en_linked_path.relative_to(
                        self.base_dir
                    )
                    updated_path = str(updated_path).replace("/en/", "/")

                print(
                    f"Redirecting link '[{text}]({path})' from {parent_dir} to {updated_path}"
                )
                return f"[{text}]({updated_path})"
            else:
                print(
                    f"Warning: Broken link '[{text}]({path})' found in {parent_dir} does not exist in /docs/en/."
                )
    ...
    def update_html_tags(content):
        ...
        content, num_replacements = re.subn(
            pattern,
            lambda match: f"![{match.group(1) or alt_tag}]({match.group(2)})",
            content,
        )
        ...
        content, num_replacements = re.subn(
            pattern,
            lambda match: match.group(0).replace(">", f' alt="{alt_tag}">', 1),
            content,
        )
    ...
    def process_markdown_file(self, md_file_path, lang_dir):
        ...
        if self.update_links:
            content = self.md_link_regex.sub(
                lambda m: self.link_replacer(m, md_file_path.parent, lang_dir), content
            )
